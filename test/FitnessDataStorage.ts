...
describe("FitnessDataStorage", function () {
  ...
  it("Should calculate average steps correctly", async function () {
    const { fitnessDataContract, fitnessDataContractAddress } = await deployFixture();

    // Set up test data for multiple workouts
    const workout1 = {
      steps: 5000,
      runningDistance: 300,
      caloriesBurned: 250,
    ...
    // Encrypt and set first workout
    const encrypted1 = await fhevm
      .createEncryptedInput(fitnessDataContractAddress, signers.alice.address)
      .add64(workout1.steps)
      .add64(workout1.runningDistance)
      .add64(workout1.caloriesBurned)
      .add64(workout1.workoutDuration)
      .add64(workout1.heartRateAvg)
      .encrypt();
    ...
    // Encrypt and set second workout
    const encrypted2 = await fhevm
      .createEncryptedInput(fitnessDataContractAddress, signers.alice.address)
      .add64(workout2.steps)
      .add64(workout2.runningDistance)
      .add64(workout2.caloriesBurned)
      .add64(workout2.workoutDuration)
      .add64(workout2.heartRateAvg)
      .encrypt();
    ...
    const averageHandle = await fitnessDataContract.getAverageSteps(signers.alice.address);
    const decryptedAverage = await fhevm.userDecryptEuint(
      FhevmType.euint64,
      averageHandle,
      fitnessDataContractAddress,
      signers.alice,
    ...
    // Expected average: (5000 + 7000) / 2 = 6000
    expect(decryptedAverage).to.eq(6000);
  });
});
